...
import io.adamnfish.pokerdot.{PokerGenerators, TestHelpers, TestTime}
...
class GamesTest extends AnyFreeSpec with Matchers with ScalaCheckDrivenPropertyChecks with TestHelpers with OptionValues with TryValues with PokerGenerators {
  "newGame" - {
    "initialises the basic fields correctly" in {
      forAll(dateGen) { now =>
        forAll { (gameName: String, trackStacks: Boolean, seed: Long) =>
          newGame(gameName, trackStacks, now, seed) should have(
    ...
    "sets expiry based on the time generated by the provided time" in {
      forAll(dateGen) { now =>
        val game = newGame("game name", false, now, 0)
        game.expiry shouldEqual expiryTime(now)
      }
    }
    ...
    "sets startTime to the current time as generated by the provided Clock implementation" in {
      forAll(dateGen) { now =>
        val game = newGame("game name", false, now, 0)
        game.startTime shouldEqual now
      }
    }

    "sets roundType to pre-flop" in {
      val game = newGame("gameName", true, 1000L, 12345L)
      game.round.phase shouldEqual PreFlop
    }

    "cards used for this round are equal if the seed is equal" in {
      forAll { (seed: Long) =>
        val game1 = newGame("gameName", false, 1000L, seed)
        val game2 = newGame("gameName", false, 1000L, seed)
    ...
    "cards used for this round are (probably) different if the seed is different" in {
      ...
      forAll { (seed1: Long, seed2: Long, seed3: Long) =>
        whenever(Set(seed1, seed2, seed3).size == 3) {
          val game1 = newGame("gameName", false, 1000L, seed1)
          val game2 = newGame("gameName", false, 1000L, seed2)
          val game3 = newGame("gameName", false, 1000L, seed3)
          val game4 = newGame("gameName", false, 1000L, seed1 + 1)
    ...
    "cards can be regenerated from the current game seed" in {
      forAll { (seed: Long) =>
        val game = newGame("gameName", false, 1000L, seed)
  ...
  "newPlayer" - {
    "initialises basic fields correctly" in {
      forAll(dateGen) { now =>
        forAll { (gid: String, screenName: String, isHost: Boolean, address: String) =>
          val player = newPlayer(GameId(gid), screenName, isHost, PlayerAddress(address), now)
          player should have(
            "gameId" as gid,
    ...
    "sets expiry based on the time generated by the provided Clock implementation" in {
      forAll(dateGen) { now =>
        val player = newPlayer(GameId("gid"), "screenName1", false, PlayerAddress("address1"), now)
        player.expiry shouldEqual expiryTime(now)
      }
    }

    "produces a different player ID each time it is called" in {
      val player1 = newPlayer(GameId("gid"), "screenName1", false, PlayerAddress("address1"), 1000L)
      val player2 = newPlayer(GameId("gid"), "screenName2", false, PlayerAddress("address2"), 1000L)
  ...
  "newSpectator" - {
    "initialises basic fields correctly" in {
      forAll { (gid: String, screenName: String, isHost: Boolean, address: String) =>
        val spectator = newSpectator(GameId(gid), screenName, isHost, PlayerAddress(address), 1000L)
    ...
    "sets expiry to the time generated by the provided Clock implementation" in {
      forAll(dateGen) { now =>
        val spectator = newSpectator(GameId("gid"), "screenName1", false, PlayerAddress("address1"), now)
        spectator.expiry shouldEqual expiryTime(now)
      }
    }

    "produces a different player ID each time it is called" in {
      val spectator1 = newSpectator(GameId("gid"), "screenName1", false, PlayerAddress("address1"), 1000L)
      val spectator2 = newSpectator(GameId("gid"), "screenName2", false, PlayerAddress("address2"), 1000L)
  ...
  "updatePlayerAddress" - {
    "if the address has changed" - {
      "updates the player address as instructed" in {
        ...
        val player = newPlayer(GameId("gid"), "screenName", false, PlayerAddress("address"), 1000L)
      ...
      "doesn't change anything else" in {
        ...
        val newAddress = PlayerAddress("address-2")
        val player = newPlayer(GameId("gid"), "screenName", false, oldAddress, 1000L)
    ...
    "if the address has not changed, returns None" in {
      ...
      val player = newPlayer(GameId("gid"), "screenName", false, playerAddress, 1000L)
  ...
  "addPlayerIds" - {
    ...
    val spectator1 = newSpectator(game.gameId, "spectator-1", false, PlayerAddress("spectator-address-1"), 1002L)
    val spectator1Db = Representations.spectatorToDb(spectator1)
    val spectator2 = newSpectator(game.gameId, "spectator-2", false, PlayerAddress("spectator-address-2"), 1003L)
  ...
  "addPlayer" - {
    val game = newGame("game name", false, 1000L, 123L)
    val player = newPlayer(game.gameId, "player name", false, PlayerAddress("address"), 1001L)
    ...
    "includes passed player as well as any existing players" in {
      val player2 = newPlayer(game.gameId, "player 2", false, PlayerAddress("address 2"), 1002L)
  ...
  "start" - {
    ...
    "sets up some key fields" in {
      forAll { (now: Long) =>
        start(game, now, None, None, None, game.players.map(_.playerId)) should have(
          "started" as true,
          "startTime" as now,
          "button" as 0,
        )
    ...
    "sets up a round" - {
      "deals the same cards for the same seed" in {
        forAll { (initialSeed: Long) =>
          val seededGame = newGame("game name", false, 0L, initialSeed)
    ...
    "sets up the players properly" - {
      val players = List(
        newPlayer(game.gameId, "player-1", false, PlayerAddress("address-1"), 0L),
        newPlayer(game.gameId, "player-2", false, PlayerAddress("address-2"), 0L),
        newPlayer(game.gameId, "player-3", false, PlayerAddress("address-3"), 0L),
        newPlayer(game.gameId, "player-4", false, PlayerAddress("address-4"), 0L),
      )
  ...
  "updateBlindAction" - {
    ...
    "for a timer update" - {
      "returns update timer action" in {
        updateBlindAction[Try](rawUpdateGame.copy(
          timerLevels = Some(List(RoundLevel(100, 10), BreakLevel(50)))
        )).success.value shouldEqual EditTimerSummary()
      }
    ...
    "for a playing status update" - {
      "returns 'playing' timer status action if the timer was started" in {
        updateBlindAction[Try](rawUpdateGame.copy(
          playing = Some(true),
        )).success.value shouldEqual TimerStatusSummary(true)
      }

      "returns 'paused' timer status action if the timer was stopped" in {
        updateBlindAction[Try](rawUpdateGame.copy(
          playing = Some(false),
        )).success.value shouldEqual TimerStatusSummary(false)
      }
    }
    ...
    "returns update blind status for a manual blind edit" in {
      updateBlindAction[Try](rawUpdateGame.copy(
        smallBlind = Some(10),
      )).success.value shouldEqual EditBlindSummary()
    }

    "returns an error if the request doesn't include any actions" in {
      updateBlindAction[Try](rawUpdateGame).isFailure shouldEqual true
    }
  }
  ...
  "resetPlayerForNextPhase" - {
    ...
    val player = newPlayer(gameId, "player", false, PlayerAddress("address"), 0L)
  ...
  "requireGame" - {
    ...
    "fails with a note about the GID, if the game wasn't found" in {
      requireGame[Try](None, "GID") match {
        case util.Failure(failures: Failures) => failures.logString should include("GID")
        case unexpected => fail(s"Expected app Failures, got $unexpected")
      }
    }
  }
  ...
  "ensureNotStarted" - {
    "is successful if the game has not yet started" in {
      val game = newGame("game name", false, 0L, 123L)
      ensureNotStarted[Try](
  ...
  "ensureStarted" - {
    ...
    "is successful if the game has begun" in {
      ...
      ensureStarted[Try](started).success.value shouldEqual ()
    }

    "fails if the game has not begun" in {
      ensureStarted[Try](game).isFailure shouldEqual true
    }
  }
  ...
  "ensureNoDuplicateScreenName" - {
    ...
    "is fine for a game with no players" in {
      forAll { (screenName: String) =>
        ensureNoDuplicateScreenName[Try](game, screenName).isSuccess shouldEqual true
      }
    }
    ...
    "is successful for a screen name that isn't already taken" in {
      forAll { (screenName: String) =>
        whenever(screenName != "screenname") {
          val player = newPlayer(game.gameId, "screenname", false, PlayerAddress("address"), 0L)
          val gameWithPlayer = addPlayer(game, player)
          ensureNoDuplicateScreenName[Try](gameWithPlayer, screenName).isSuccess shouldEqual true
        }
      }
    }
    ...
    "fails for a screen name that is already in use in this game" in {
      forAll { (screenName: String) =>
        val player = newPlayer(game.gameId, screenName, false, PlayerAddress("address"), 0L)
        val gameWithPlayer = addPlayer(game, player)
        ensureNoDuplicateScreenName[Try](gameWithPlayer, screenName).isFailure shouldEqual true
      }
    }
  }
  ...
  "ensurePlayerCount" - {
    "succeeds if there are a sensible number of players" in {
      forAll(Gen.choose(0, 19)) { n =>
        ensurePlayerCount[Try](n).success.value shouldEqual ()
      }
    }

    "fails when there are already 20 players" in {
      ensurePlayerCount[Try](20).isFailure shouldEqual true
    }

    "fails if there are somehow already more than 20 players" in {
      forAll(Gen.choose(21, 50)) { n =>
        ensurePlayerCount[Try](n).isFailure shouldEqual true
      }
    }
  }
  ...
  "ensureStartingPlayerCount" - {
    "succeeds if the player count is more than 1" in {
      forAll(Gen.choose(2, 20)) { n =>
        ensureStartingPlayerCount[Try](n).success.value shouldEqual ()
      }
    }

    "fails if ther is only one player in the game" in {
      ensureStartingPlayerCount[Try](1).isFailure shouldEqual true
    }
  }
  ...
  "ensureNotAlreadyPlaying" - {
    ...
    "succeeds for a player address that isn't already in use" in {
      forAll { (address: String) =>
        whenever(address != "address1") {
          val player = newPlayer(game.gameId, "screenname", false, PlayerAddress("address1"), 0L)
          val gameWithPlayer = addPlayer(game, player)
          ensureNotAlreadyPlaying[Try](gameWithPlayer.players, PlayerAddress(address)).isSuccess shouldEqual true
        }
      }
    }
    ...
    "fails for a player address that is already being used" in {
      forAll { (address: String) =>
        val player = newPlayer(game.gameId, "screenname", false, PlayerAddress(address), 0L)
        val gameWithPlayer = addPlayer(game, player)
        ensureNotAlreadyPlaying[Try](gameWithPlayer.players, PlayerAddress(address)).isFailure shouldEqual true
      }
    }
  }
  ...
  "ensurePlayerKey" - {
    val game = newGame("game name", false, 0L, 123L)
    val player = newPlayer(game.gameId, "player name", false, PlayerAddress("address"), 0L)

    "if the player is part of this game" - {
      val gameWithPlayer = addPlayer(game, player)

      "returns the valid player if their key is valid" in {
        val playerResult = ensurePlayerKey[Try](gameWithPlayer.players, player.playerId, player.playerKey).success.value
        playerResult shouldEqual player
      }

      "fails if the player key does not match" in {
        val incorrectPlayerKey = PlayerKey("bad player key")
        val result = ensurePlayerKey[Try](gameWithPlayer.players, player.playerId, incorrectPlayerKey)
        result.isFailure shouldEqual true
      }
    }

    "fails if the player does not exist in the game" in {
      val result = ensurePlayerKey[Try](game.players, player.playerId, player.playerKey)
      result.isFailure shouldEqual true
    }
  }
  ...
  "ensureSpectatorKey" - {
    val game = newGame("game name", false, 0L, 123L)
    val spectator = newSpectator(game.gameId, "player name", false, PlayerAddress("address"), 0L)
    ...
    "if the spectator is part of this game" - {
      ...
      "returns the valid spectator if their key is valid" in {
        val spectatorResult = ensureSpectatorKey[Try](gameWithPlayer.spectators, spectator.playerId, spectator.playerKey).success.value
        spectatorResult shouldEqual spectator
      }

      "fails if the spectator's player key does not match" in {
        val incorrectPlayerKey = PlayerKey("bad player key")
        val result = ensureSpectatorKey[Try](gameWithPlayer.spectators, spectator.playerId, incorrectPlayerKey)
        result.isFailure shouldEqual true
      }
    }

    "fails if the spectator does not exist in the game" in {
      val result = ensureSpectatorKey[Try](game.spectators, spectator.playerId, spectator.playerKey)
      result.isFailure shouldEqual true
    }
  }
  ...
  "ensureHost" - {
    val gameId = GameId("game-id")
    val host = newPlayer(gameId, "host", true, PlayerAddress("host-address"), 0L)
    ...
    "succeeds if the provided player is the host" in {
      ensureHost[Try](players, host.playerKey).success.value shouldEqual host
    }

    "fails if the provided player is not the host" in {
      ensureHost[Try](players, player1.playerKey).isFailure shouldEqual true
    }

    "fails if the provided player does not exist in the game" in {
      val nonPlayerKey = PlayerKey("not-in-the-game")
      ensureHost[Try](players, nonPlayerKey).isFailure shouldEqual true
    }
  }

  "ensureAdmin" - {
    val gameId = GameId("game-id")
    val admin = newPlayer(gameId, "host", true, PlayerAddress("host-address"), 0L)
    val player1 = newPlayer(gameId, "player-1", false, PlayerAddress("player-1-address"), 0L)
  ...
  "ensureActive" - {
    "succeeds if the player is active" in {
      ...
      ensureActive[Try](Some(pid), pid).isSuccess shouldEqual true
    }
    ...
    "fails if no player is active" in {
      ensureActive[Try](None, PlayerId("player")).isFailure shouldEqual true
    }
  }
}
