...
class GamesTest extends AnyFreeSpec with Matchers with ScalaCheckDrivenPropertyChecks with TestHelpers with EitherValues with OptionValues with PokerGenerators {
  "newGame" - {
    "initialises the basic fields correctly" in {
      forAll { (gameName: String, trackStacks: Boolean, seed: Long) =>
        newGame(gameName, trackStacks, 0L, seed) should have(
    ...
    "sets expiry based on the time generated by the provided Clock implementation" in {
      val game = newGame("game name", false, 0L, 0)
      game.expiry shouldEqual expiryTime(0)
    }

    "sets startTime to the current time as generated by the provided Clock implementation" in {
      val game = newGame("game name", false, 0L, 0)
      game.startTime shouldEqual 0
    }

    "sets roundType to pre-flop" in {
      val game = newGame("gameName", true, 0L, 12345L)
      game.round.phase shouldEqual PreFlop
    }

    "cards used for this round are equal if the seed is equal" in {
      forAll { (seed: Long) =>
        val game1 = newGame("gameName", false, 0L, seed)
        val game2 = newGame("gameName", false, 0L, seed)
    ...
    "cards used for this round are (probably) different if the seed is different" in {
      ...
      forAll { (seed1: Long, seed2: Long, seed3: Long) =>
        whenever(Set(seed1, seed2, seed3).size == 3) {
          val game1 = newGame("gameName", false, 0L, seed1)
          val game2 = newGame("gameName", false, 0L, seed2)
          val game3 = newGame("gameName", false, 0L, seed3)
          val game4 = newGame("gameName", false, 0L, seed1 + 1)
    ...
    "cards can be regenerated from the current game seed" in {
      forAll { (seed: Long) =>
        val game = newGame("gameName", false, 0L, seed)
  ...
  "newPlayer" - {
    "initialises basic fields correctly" in {
      forAll { (gid: String, screenName: String, isHost: Boolean, address: String) =>
        val player = newPlayer(GameId(gid), screenName, isHost, PlayerAddress(address), 0L)
        player should have(
          "gameId" as gid,
    ...
    "sets expiry based on the time generated by the provided Clock implementation" in {
      val player = newPlayer(GameId("gid"), "screenName1", false, PlayerAddress("address1"), 0L)
      player.expiry shouldEqual expiryTime(0)
    }

    "produces a different player ID each time it is called" in {
      val player1 = newPlayer(GameId("gid"), "screenName1", false, PlayerAddress("address1"), 0L)
      val player2 = newPlayer(GameId("gid"), "screenName2", false, PlayerAddress("address2"), 0L)
  ...
  "newSpectator" - {
    "initialises basic fields correctly" in {
      forAll { (gid: String, screenName: String, isHost: Boolean, address: String) =>
        val spectator = newSpectator(GameId(gid), screenName, isHost, PlayerAddress(address), 0L)
    ...
    "sets expiry to the time generated by the provided Clock implementation" in {
      val spectator = newSpectator(GameId("gid"), "screenName1", false, PlayerAddress("address1"), 0L)
      spectator.expiry shouldEqual expiryTime(0)
    }

    "produces a different player ID each time it is called" in {
      val spectator1 = newSpectator(GameId("gid"), "screenName1", false, PlayerAddress("address1"), 0L)
      val spectator2 = newSpectator(GameId("gid"), "screenName2", false, PlayerAddress("address2"), 0L)
  ...
  "updatePlayerAddress" - {
    "if the address has changed" - {
      "updates the player address as instructed" in {
        ...
        val player = newPlayer(GameId("gid"), "screenName", false, PlayerAddress("address"), 0L)
      ...
      "doesn't change anything else" in {
        ...
        val newAddress = PlayerAddress("address-2")
        val player = newPlayer(GameId("gid"), "screenName", false, oldAddress, 0L)
    ...
    "if the address has not changed, returns None" in {
      ...
      val player = newPlayer(GameId("gid"), "screenName", false, playerAddress, 0L)
  ...
  "addPlayerIds" - {
    ...
    val player1 = newPlayer(game.gameId, "player name", false, PlayerAddress("address 1"), 0L)
    val player1Db = Representations.playerToDb(player1)
    val player2 = newPlayer(game.gameId, "player 2 name", false, PlayerAddress("address 2"), 0L)
    val player2Db = Representations.playerToDb(player2)
    val spectator1 = newSpectator(game.gameId, "spectator-1", false, PlayerAddress("spectator-address-1"), 0L)
    val spectator1Db = Representations.spectatorToDb(spectator1)
    val spectator2 = newSpectator(game.gameId, "spectator-2", false, PlayerAddress("spectator-address-2"), 0L)
  ...
  "addPlayer" - {
    ...
    "includes passed player as well as any existing players" in {
      val player2 = newPlayer(game.gameId, "player 2", false, PlayerAddress("address 2"), 0L)
