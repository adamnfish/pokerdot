...
class GamesTest extends AnyFreeSpec with Matchers with ScalaCheckDrivenPropertyChecks with TestHelpers with EitherValues {
  "newGame" - {
    "initialises the basic fields correctly" in {
      forAll { (gameName: String, trackStacks: Boolean, seed: Long) =>
        newGame(gameName, trackStacks, TestDates).value(seed) should have(
    ...
    "sets expiry to the time generated by the provided Dates implementation" in {
      val game = newGame("game name", false, TestDates).value(0)
    ...
    "sets roundType to pre-flop" in {
      val game = newGame("gameName", true, TestDates).value(12345L)
      game.round.phase shouldEqual PreFlop
    }

    "cards used for this round are equal if the seed is equal" in {
      forAll { (seed: Long) =>
        val game1 = newGame("gameName", false, TestDates).value(seed)
        val game2 = newGame("gameName", false, TestDates).value(seed)
    ...
    "cards used for this round are (probably) different if the seed is different" in {
      ...
      forAll { (seed1: Long, seed2: Long, seed3: Long) =>
        whenever(Set(seed1, seed2, seed3).size == 3) {
          val game1 = newGame("gameName", false, TestDates).value(seed1)
          val game2 = newGame("gameName", false, TestDates).value(seed2)
          val game3 = newGame("gameName", false, TestDates).value(seed3)
          val game4 = newGame("gameName", false, TestDates).value(seed1 + 1)
    ...
    "cards can be regenerated from the current game seed" in {
      forAll { (seed: Long) =>
        val game = newGame("gameName", false, TestDates).value(seed)
  ...
  "addPlayerIds" - {
    val game = newGame("game name", false, TestDates).value(123L)
  ...
  "requireGame" - {
    "returns the gameDb if present" in {
      val gameDb = Representations.gameToDb(
        newGame("game name", false, TestDates).value(123L)
      )
